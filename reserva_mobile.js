  ...
  for(const h of horarios){
    ...
    for(const qid of quadrasOrder){
      ...
      if(info && info.ativa){
        ...
        topline.innerHTML = `<span>${escapeHtml(isMobileMode() ? shortNameForMobile(cliNome) : cliNome)}</span><span>${statusIcon(r.status_reserva)}</span>`;
...
badge.className = `badge ${String(r.tipo_reserva).toLowerCase() === 'fixo' ? 'badge-fixo' : 'badge-normal'}`;
badge.textContent = (String(r.tipo_reserva || 'Normal').toLowerCase() === 'fixo') ? 'Fixo' : 'Normal';
